...
mod tests {
    ...
    fn os_predicates_match_cfg() {
        ...
        assert_eq!(is_windows(&no_args).unwrap(), Value::Bool(cfg!(target_os = "windows")));
        assert_eq!(is_linux(&no_args).unwrap(), Value::Bool(cfg!(target_os = "linux")));
        assert_eq!(is_mac(&no_args).unwrap(), Value::Bool(cfg!(target_os = "macos")));
